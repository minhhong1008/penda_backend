...
// Hàm tạo ra liên kết các account: Tạo info liên kết full với các field sau đó info ghép nối với device, lưu ý list_view full các field
          ...
          list_view.forEach((view) => {
            if (!item[view]) {
              ...
              if (view == "payoneer_id") {
                ...
                console.log(item[view]);
                Payoneer.findOne({ payoneer_id: item[view] }).exec(
                  (err, item_field) => {
                    if (item_field) {
                      item[view] = _id;
                      console.log(item[view]);
